...
// Fetch user, college, and course details from the database
$sql = "SELECT 
            users.*, 
            colleges.name AS college_name, 
            courses.name AS course_name
        FROM users
        LEFT JOIN colleges ON users.college_id = colleges.college_id
        LEFT JOIN college_courses ON colleges.college_id = college_courses.college_id
        LEFT JOIN courses ON college_courses.course_id = courses.course_id
        WHERE users.user_id = ?";
            ...
            <div class="bg-white p-6 rounded-lg shadow-lg border border-gray-200">
                ...
                <div class="flex flex-col md:flex-row">
                    ...
                    <div class="md:w-1/2 p-4">
                        ...
                        <label class="block mt-2 text-sm font-medium text-gray-700">Course Name</label>
                        <input type="text" class="w-full mt-1 p-2 border rounded-md" value="<?= htmlspecialchars($user['course_name']); ?>" readonly />
                        <label class="block mt-2 text-sm font-medium text-gray-700">College Name</label>
                        <input type="text" class="w-full mt-1 p-2 border rounded-md" value="<?= htmlspecialchars($user['college_name']); ?>" readonly />
                    </div>
                </div>
