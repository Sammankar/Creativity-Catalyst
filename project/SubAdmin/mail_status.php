...
function sendGuideStatusEmail($guide_id, $changed_by, $new_status, $subject, $body, $warning, $attachments) {
    $mail = new PHPMailer(true);
    $uploadDir = __DIR__ . '/images/guide_status_reports/';  // Correct path
  // Make sure this path is correct
    ...
    try {
        ...
        $stmt = $db->prepare("INSERT INTO guide_status_reports (guide_id, changed_by, old_status, new_status, subject, body, warning, attachments) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $jsonAttachments = json_encode($attachmentPaths);
        $stmt->bind_param("iiiissss", $guide_id, $changed_by, $old_status, $new_status, $subject, $body, $warning, $jsonAttachments);
        $stmt->execute();
        $stmt->close();
