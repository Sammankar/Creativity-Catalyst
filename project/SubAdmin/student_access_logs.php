...
$query = "SELECT 
          ...
          WHERE student.college_id = ?";  // Filter by college_id of the logged-in admin
...
<div class="bg-gradient-to-r from-blue-400 to-teal-400 p-6 shadow-lg rounded-lg mb-6">
    <form method="GET" action="" class="flex flex-wrap gap-4 items-center">
        ...
        <a href="student_access_logs.php" class="bg-gray-500 text-white px-5 py-3 rounded-lg shadow-xl hover:bg-gray-600 transition-all duration-300">Reset</a>
    </form>
    ...
    <div id="report-list" class="grid grid-cols-1 gap-6 mt-6 max-h-[500px] overflow-y-auto p-4 justify-items-center">
    <?php if (count($reports) > 0): ?>
        <?php foreach ($reports as $report) : ?>
            <div class="bg-gradient-to-r from-blue-100 via-teal-100 to-blue-50 w-full p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 flex justify-between items-center max-w-full">
                ...
                <!-- Center Section: Ongoing/Resolved & Action Button -->
                <div class="flex flex-col justify-center items-center w-1/3 space-y-4">
                    ...
                    <?php if ($report['access_denial_report_status'] == 0): ?>
                        <div class="flex space-x-4">
                            <button class="give-access-btn bg-blue-500 text-white px-4 py-2 rounded-lg shadow-md hover:bg-blue-600 transition" data-report-id="<?php echo $report['id']; ?>" data-user-id="<?php echo $report['user_id']; ?>">Grant</button>
                            <button class="restrict-access-btn bg-green-500 text-white px-4 py-2 rounded-lg shadow-md hover:bg-green-600 transition" data-report-id="<?php echo $report['id']; ?>" data-user-id="<?php echo $report['user_id']; ?>">Restrict</button>
                        </div>
                    <?php else: ?>
                        ...
                        <button class="access-update-btn bg-yellow-500 text-white px-3 py-3 rounded-lg shadow-xl hover:bg-yellow-600 transition-all duration-300 mt-4"
                                data-report-id="<?php echo $report['id']; ?>" data-user-id="<?php echo $report['user_id']; ?>">Access Update</button>
                    <?php endif; ?>
                </div>
...
<div id="accessUpdatePopup" class="fixed top-0 left-0 w-full h-full bg-black bg-opacity-60 flex justify-center items-center hidden">
    <div class="bg-white p-6 rounded-lg shadow-lg w-80">
        ...
        <button class="bg-gray-500 text-white px-5 py-3 rounded-lg shadow-xl hover:bg-gray-600 transition-all close-popup-btn">Cancel</button>    </div>
</div>

<script>
        function expandReport(reportId) {
            window.location.href = "student_report_details.php?id=" + reportId;
        }
        function openChat(reportId) {
            window.location.href = "chat.php?id=" + reportId;
        }
    </script>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        document.querySelectorAll(".give-access-btn, .restrict-access-btn").forEach(button => {
            button.addEventListener("click", function () {
                ...
                fetch("update_student_report_access_status.php", {
                    method: "POST",
                    headers: { "Content-Type": "application/x-www-form-urlencoded" },
                    body: `id=${reportId}&user_id=${userId}&action=${action}`
                })
    ...
    fetch("student_get_message.php?report_id=" + reportId)
        .then(response => response.json())
        .then(data => {
            ...
            fetch("student_access_logs.php?report_id=" + reportId)
                .then(response => response.json())
                .then(data => {
                    if (data.locked) {
    ...
    function sendMessage() {
        ...
        fetch("student_send_message.php", {
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateAccess(reportId, userId, action) {
        fetch("update_student_report_access_status_v2.php", { // Use the new PHP file here
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            body: `id=${reportId}&user_id=${userId}&action=${action}`
        })
    ...
    // Ensure that the Cancel button closes the popup
