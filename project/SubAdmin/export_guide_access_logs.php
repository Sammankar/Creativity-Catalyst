...
$query = "SELECT 
            ...
            adr.access_denial_report_status
          FROM guide_access_denial_reports adr
          JOIN users sa ON adr.changed_by = sa.user_id
          JOIN users student ON adr.user_id = student.user_id
...
$pdf->Cell(40, 10, 'Guide Name', 1, 0, 'C');
$pdf->Cell(40, 10, 'Sub-Admin Name', 1, 0, 'C');
...
foreach ($reports as $report) {
    ...
    $pdf->Cell(40, 10, $report['user_name'], 1, 0, 'C');  // Sub-admin name
    $pdf->Cell(40, 10, $report['changed_by_name'], 1, 0, 'C');  // Admin name (who changed the access)
