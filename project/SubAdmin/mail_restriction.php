...
function sendAccessChangeEmail($guide_id, $changed_by, $new_access, $subject, $body, $warning, $attachments) {
    $mail = new PHPMailer(true);
    $uploadDir = __DIR__ . '/images/guide_access_reports/';
    ...
    try {
        ...
        $stmt = $db->prepare("INSERT INTO guide_access_denial_reports (user_id, changed_by, old_access, new_access, subject, body, warning, attachment) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        // If no files, insert null for attachment field
        $jsonAttachments = ($attachmentPaths !== null) ? json_encode($attachmentPaths) : null;
        $stmt->bind_param("iiiissss", $guide_id, $changed_by, $old_access, $new_access, $subject, $body, $warning, $jsonAttachments);
        $stmt->execute();
        if ($stmt->error) {
        ...
        $query = "INSERT INTO guide_chat_reports (report_id, locked) VALUES (?, 0)";
        ...
        $stmt->bind_param("ii", $new_access, $guide_id);
        $stmt->execute();
        $stmt->close();
