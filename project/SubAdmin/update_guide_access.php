...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    require 'mail_restriction.php';
    
    // Update access status
    $stmt = $conn->prepare("UPDATE users SET access_status = ? WHERE user_id = ?");
    $stmt->bind_param("ii", $access_action, $guide_id);
    $stmt->execute();
    ...
    // Send email
    $emailResult = sendAccessChangeEmail($guide_id, $changed_by, $access_action, $subject, $body, $warning, $_FILES['attachments'] ?? null);
