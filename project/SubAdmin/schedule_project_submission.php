...
if ($validCourse) {
    ...
    $query = "
        ...
        WHERE ac.course_id = ? $searchCondition
        AND NOT EXISTS (
            SELECT 1 FROM project_submission_schedule pss
            WHERE pss.academic_calendar_id = ac.id 
            AND pss.course_id = ac.course_id
            AND pss.semester = ac.semester
            AND pss.academic_year = ac.academic_year
        )
    ...
    $stmt = $conn->prepare("
        ...
        INNER JOIN academic_calendar ac ON ac.id = pss.academic_calendar_id
        WHERE ac.course_id = ? AND pss.course_id = ? AND pss.college_id = ?
    ");
    $stmt->bind_param("iii", $course_id, $course_id, $college_id);
    $stmt->execute();
    $completedSchedule = $stmt->get_result()->fetch_assoc()['completed'] ?? 0;
    $stmt->close();
    ...
    $stmt = $conn->prepare("
        ...
        FROM academic_calendar ac
        WHERE ac.course_id = ? AND NOT EXISTS (
            SELECT 1 FROM project_submission_schedule pss
            WHERE pss.academic_calendar_id = ac.id
            AND pss.course_id = ac.course_id
            AND pss.semester = ac.semester
    ...
    $stmt->bind_param("i", $course_id);
    $stmt->execute();
    $pendingSchedule = $stmt->get_result()->fetch_assoc()['pending'] ?? 0;
    $stmt->close();
    ...
    $completedSchedule = $pendingSchedule = 0;
}
...
<a 
  ...
  style="background-color: #ffff;"
  data-id="<?php echo $row['id']; ?>"
  onclick="redirectToSchedule(this)">
  Project
</button>
              </td>
        </tr>
...
<script>
  function redirectToSchedule(button) {
    const id = button.getAttribute('data-id');
    if (id) {
      // Redirect with query parameter
      window.location.href = `create_project_schedule.php?id=${id}`;
    }
  }
</script>
