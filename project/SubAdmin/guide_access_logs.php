...
$query = "SELECT 
          ...
          WHERE guide.college_id = ?";  // Filter by college_id of the logged-in admin
...
<div class="bg-gradient-to-r from-blue-400 to-teal-400 p-6 shadow-lg rounded-lg mb-6">
    <form method="GET" action="" class="flex flex-wrap gap-4 items-center">
        ...
        <a href="guide_access_logs.php" class="bg-gray-500 text-white px-5 py-3 rounded-lg shadow-xl hover:bg-gray-600 transition-all duration-300">Reset</a>
    </form>
    <a href="export_guide_access_logs.php" class="bg-red-500 text-white px-5 py-3 rounded-lg shadow-xl mt-3 inline-block hover:bg-red-600 transition-all duration-300">Export as PDF</a>
        ...
        <?php foreach ($reports as $report) : ?>
            <div class="bg-gradient-to-r from-blue-100 via-teal-100 to-blue-50 w-full p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 flex justify-between items-center max-w-full">
                ...
                <div class="flex flex-col space-y-3 w-2/5">
                    ...
                    <p class="text-sm text-gray-600">Guide Name: 
                ...
                <div class="flex flex-col space-y-4 justify-center w-1/3">
                    ...
                    <button class="bg-green-500 text-white px-6 py-3 rounded-lg shadow-md hover:bg-green-600 transition duration-300"
                            onclick="openChat(<?php echo $report['report_id']; ?>, <?php echo $report['admin_id']; ?>)">
                        Chat
                    </button>
                </div>
            </div>
...
<script>
        function expandReport(reportId) {
            window.location.href = "guide_report_details.php?report_id=" + reportId;
        }
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        document.querySelectorAll(".give-access-btn, .restrict-access-btn").forEach(button => {
            button.addEventListener("click", function () {
                ...
                fetch("update_guide_report_access_status.php", {
                    method: "POST",
    ...
    fetch("guide_get_message.php?report_id=" + reportId)
        .then(response => response.json())
        .then(data => {
            ...
            fetch("guide_access_logs.php?report_id=" + reportId)
                .then(response => response.json())
                .then(data => {
                    if (data.locked) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateAccess(reportId, userId, action) {
        fetch("update_guide_report_access_status_v2.php", { // Use the new PHP file here
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            body: `report_id=${reportId}&user_id=${userId}&action=${action}`
        })
