...
$filter = isset($_GET['filter']) ? $_GET['filter'] : 'subadmin';

// Fetch college_id of currently logged-in Admin
$loggedInUserId = $_SESSION['user_id'];
$college_id = 0;
$getCollege = $conn->query("SELECT college_id FROM users WHERE user_id = $loggedInUserId");
if ($getCollege->num_rows > 0) {
  $college_id = $getCollege->fetch_assoc()['college_id'];
}
?>
...
<main class="h-full overflow-y-auto bg-gray-50 dark:bg-gray-900">
  <div class="container px-4 py-6 mx-auto grid max-w-5xl">
    ...
    <div class="mb-6">
      ...
      <select id="notificationType" onchange="filterNotifications(this.value)"
        ...
        <option value="subadmin" <?php echo ($filter == 'subadmin') ? 'selected' : ''; ?>>Sub-admin Login Requests</option>
        <option value="course" <?php echo ($filter == 'course') ? 'selected' : ''; ?>>Course Requests</option>
    ...
    <?php if ($filter == 'subadmin') { ?>
    <!-- Sub-admin Login Request Notifications -->
    <div id="subadmin-notifications" class="space-y-4 overflow-y-auto">
  <?php
  // Mark unread sub-admin login requests as read
  $conn->query("UPDATE users SET is_read_by = 1 WHERE role = 3 AND college_id = $college_id AND is_read_by = 0");

  // Fetch sub-admin login requests
  $query = "SELECT user_id, full_name, email, access_status, users_status, created_at, is_read_by 
            FROM users 
            WHERE role = 3 AND college_id = $college_id
  ...
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
      ...
      ?>
      <a href="sub_admin_list.php" class="block">
    ...
    echo "<p class='text-gray-600 dark:text-gray-300'>No Sub-admin Login Requests found.</p>";
  }
  ?>
</div>
<?php } ?>

 <!-- Placeholder divs for other notification types -->
    <!-- Course Request Notifications -->
    <?php if ($filter == 'course') { ?>
      <div id="course-notifications" class="space-y-4">
        <?php
        // Step-4: Mark unread course requests as read
        $conn->query("UPDATE course_requests SET is_read_by_superadmin = 1 WHERE is_read_by_superadmin = 0");

        $query = "SELECT cr.*, u.full_name, u.email, c.name as college_name, cs.name as course_name 
                  FROM course_requests cr
                  JOIN users u ON cr.admin_id = u.user_id
                  JOIN colleges c ON cr.college_id = c.college_id
                  JOIN courses cs ON cr.requested_course_id = cs.course_id
        ...
        if ($result->num_rows > 0) {
          while ($row = $result->fetch_assoc()) {
            ...
            // Set visual cue for read/unread
            $border_class = ($row['is_read_by_superadmin'] == 0) ? 'border-2 border-blue-600' : 'border border-blue-200';
        ?>
          <a href="request_course_logs.php" class="block">
            <div class="p-6 bg-white rounded-2xl shadow hover:shadow-lg transition-all duration-200 dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-gray-700 <?php echo $border_class; ?>">
              <div class="flex justify-between items-center mb-2">
                ...
                <span class="text-sm px-2 py-1 rounded-full <?php echo $status_class; ?>">
              ...
              <p class="text-sm text-gray-700 dark:text-gray-200 mt-2">
                <span class="font-medium">College:</span> <?php echo htmlspecialchars($row['college_name']); ?><br>
                <span class="font-medium">Requested Course:</span> <?php echo htmlspecialchars($row['course_name']); ?>
              </p>
        ...
        <?php
          ...
          echo "<p class='text-gray-600 dark:text-gray-300'>No Course Requests found.</p>";
        }
        ?>
      </div>
    <?php } ?>
...
<script>
  function filterNotifications(type) {
    const types = ["admin", "course", "calendar", "competition"];
    types.forEach(t => {
      document.getElementById(`${t}-notifications`).style.display = t === type ? "block" : "none";
    });
  }
</script>
<script>
