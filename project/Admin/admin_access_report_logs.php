...
$query = "SELECT 
            ...
            sa.user_id AS super_admin_id  -- Fetch super-admin ID
          FROM access_denial_reports adr
          JOIN users sa ON adr.changed_by = sa.user_id AND sa.role = 1
          JOIN users admin ON adr.user_id = admin.user_id AND admin.role = 2
...
<head>
    ...
    <title>Admin Access Reports</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100">
    <div class="max-w-6xl mx-auto p-4">
        <h1 class="text-3xl font-extrabold text-gray-800 mb-6">Admin Access Reports</h1>
    ...
    <form method="GET" action="" class="flex flex-wrap gap-4 items-center">
        ...
        <a href="admin_access_report_logs.php" class="bg-gray-500 text-white px-5 py-3 rounded-lg shadow-xl hover:bg-gray-600 transition-all duration-300">Reset</a>
    </form>
    <a href="export_admin_access_logs.php" class="bg-red-500 text-white px-5 py-3 rounded-lg shadow-xl mt-3 inline-block hover:bg-red-600 transition-all duration-300">Export as PDF</a>
...
<div id="report-list" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-6 justify-items-center max-h-[500px] overflow-y-auto">
        ...
        <?php foreach ($reports as $report) : ?>
            <div class="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col justify-between max-w-sm mx-auto space-y-4">
                <div class="flex flex-col space-y-3">
                    ...
                    <p class="text-sm text-gray-600">Admin Name: 
                ...
                <div class="flex space-x-4 mt-4 mr-4">
                    ...
                    <button class="bg-green-500 text-white px-6 py-3 rounded-lg shadow-md hover:bg-green-600 transition duration-300"
                            onclick="openChat(<?php echo $report['report_id']; ?>, <?php echo $report['super_admin_id']; ?>)">
                        Chat
                    </button>
                </div>
            </div>
    ...
    <script>
        function expandReport(reportId) {
            window.location.href = "admin_access_report_log_details.php?report_id=" + reportId;
        }
        function openChat(reportId) {
            window.location.href = "admin_chat.php?report_id=" + reportId;
        }
    </script>
<script>
    ...
    function sendMessage() {
        ...
        let reportId = document.getElementById("chat-container").getAttribute("data-report");
        let receiverId = document.getElementById("chat-container").getAttribute("data-super-admin-id");  // Get super-admin ID dynamically
