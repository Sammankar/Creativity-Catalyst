...
// Fetch reports data for the logged-in user
$query = "SELECT 
            adr.*, 
          ...
          JOIN users evaluator ON adr.user_id = evaluator.user_id
          WHERE adr.user_id = ?  -- Filter by session user_id
          ORDER BY adr.created_at DESC";

$stmt = $conn->prepare($query);
$stmt->bind_param("i", $user_id);  // Bind session user_id to the query
$stmt->execute();
$result = $stmt->get_result();
...
$pdf->Cell(40, 10, 'Evaluator Name', 1, 0, 'C');
