...
if (!$submission || empty($submission['guide_marks'])) {
    echo "<div class='p-4 text-red-600'>No marks found to edit.</div>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: edit_marks.php?stage_id=$stage_id&student_id=$student_user_id&semester=$semester&academic_year=$academic_year&course=$course&success=1");
            exit;
        } else {
            $errors[] = "Database error: Unable to update marks.";
        }
    }
}
?>
...
<main class="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
    <div class="w-full max-w-2xl mx-auto p-4">
        <div class="max-w-3xl p-6 bg-white rounded-lg shadow-md dark:bg-gray-800">
            <h2 class="text-xl font-semibold text-center text-gray-700 dark:text-gray-200 mb-6">Edit Marks for Stage</h2>
            ...
            <div class="mb-4 mt-4">
                <strong class="text-gray-700 dark:text-gray-200">Previously Given Marks:</strong>
                <span class="text-green-700 font-semibold"><?= $submission['guide_marks'] ?></span>
            </div>
            <?php if (isset($_GET['success'])): ?>
                <div class="p-3 bg-green-200 text-green-800 rounded mb-4">Marks updated successfully!</div>
            <?php endif; ?>
            ...
            <?php if ($today >= $unlock_date && $today >= $start_date && $today <= $end_date): ?>
                <h2 class="text-xl font-semibold text-center text-gray-700 dark:text-gray-200 mt-8 mb-4">Edit Marks</h2>
                <form method="POST">
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        <label class="block font-semibold text-gray-700 dark:text-gray-200" for="guide_marks">Enter New Marks:</label>
                        <input type="number" step="0.01" min="0" max="<?= $stage['marks'] ?>" name="guide_marks" id="guide_marks"
                               value="<?= $submission['guide_marks'] ?>" required
                               class="w-full px-4 py-2 mt-1 border rounded-md focus:ring focus:ring-blue-300 dark:bg-gray-700 dark:text-white">
                    </div>

                    <div class="mb-4">
                        <label class="block font-semibold text-gray-700 dark:text-gray-200" for="guide_feedback">Edit Feedback (Optional):</label>
                        <textarea name="guide_feedback" id="guide_feedback" rows="3"
                                  class="w-full px-4 py-2 mt-1 border rounded-md focus:ring focus:ring-blue-300 dark:bg-gray-700 dark:text-white"><?= htmlspecialchars($submission['guide_feedback']) ?></textarea>
                    </div>

                    <button type="submit" class="w-full px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-600">Update Marks</button>
                </form>
            <?php else: ?>
                <div class="p-4 mt-4 text-yellow-700 bg-yellow-100 rounded-md">Marks can only be edited after unlock date and within the schedule period.</div>
            <?php endif; ?>
