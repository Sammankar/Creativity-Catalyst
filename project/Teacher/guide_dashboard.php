...
if (isset($_SESSION['user_id'])) {
  ...
  // Step 1: Fetch the Sub-Admin's college_id and full name
  $query = "SELECT college_id, full_name, course_id FROM users WHERE user_id = ?";
  $stmt = $conn->prepare($query);
  $stmt->bind_param("i", $guide_user_id);
  $stmt->execute();
  $stmt->bind_result($college_id, $guide_full_name, $course_id);
  $stmt->fetch();
  $stmt->close();
  ...
  // Step 3: Fetch Course Name from courses table
...
<main class="h-full overflow-y-auto">
    ...
    <!-- Course Overview -->
    <div class="w-full max-w-xl p-6   mt-6 mx-auto">
