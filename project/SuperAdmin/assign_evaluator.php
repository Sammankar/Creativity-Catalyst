...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['assign_evaluators'])) {
    $competition_id = $_GET['id']; // ✅ matches URL param
    $selected_evaluators = $_POST['evaluators']; // Array of selected evaluator user_ids

    if (count($selected_evaluators) >= 1 && count($selected_evaluators) <= 10) {
        foreach ($selected_evaluators as $evaluator_id) {
            $stmt = $conn->prepare("INSERT INTO evaluators (competition_id, user_id) VALUES (?, ?)");
            $stmt->bind_param("ii", $competition_id, $evaluator_id);
            $stmt->execute();
        }
        $_SESSION['message'] = "Evaluators assigned successfully!";
        $_SESSION['message_type'] = "success";
    } else {
        $_SESSION['message'] = "You must select between 1 and 10 evaluators.";
        $_SESSION['message_type'] = "error";
    }
    header("Location: competition_assign_evaluator.php?competition_id=$competition_id");
    exit();
}
