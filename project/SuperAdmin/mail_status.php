...
function sendAdminStatusEmail($admin_id, $changed_by, $new_status, $subject, $body, $warning, $attachments) {
    $mail = new PHPMailer(true);
    $uploadDir = __DIR__ . '/images/admin_status_reports/';  // Make sure this path is correct
    $attachmentPaths = [];
    ...
    try {
        ...
        $stmt = $db->prepare("INSERT INTO admin_status_reports (admin_id, changed_by, old_status, new_status, subject, body, warning, attachments) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $jsonAttachments = json_encode($attachmentPaths);
        $stmt->bind_param("iiiissss", $admin_id, $changed_by, $old_status, $new_status, $subject, $body, $warning, $jsonAttachments);
        $stmt->execute();
        $stmt->close();
