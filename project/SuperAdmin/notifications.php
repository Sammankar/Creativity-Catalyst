...
<main class="h-full overflow-y-auto bg-gray-50 dark:bg-gray-900">
  <div class="container px-4 py-6 mx-auto grid max-w-5xl">
    ...
    <div class="mb-6">
      ...
      <select id="notificationType" onchange="filterNotifications(this.value)"
        ...
        <option value="admin" <?php echo ($filter == 'admin') ? 'selected' : ''; ?>>Admin Login Requests</option>
    ...
    <?php if ($filter == 'admin') { ?>
    <!-- Admin Login Request Notifications -->
    <div id="admin-notifications" class="space-y-4 overflow-y-auto">
  <?php
  // Step-1: Mark unread admin login requests as read
  $conn->query("UPDATE users SET is_read_by = 1 WHERE role = 2 AND is_read_by = 0");

  // Step-2: Fetch admin login requests
  ...
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
      ...
      // Step-3: Set visual border based on read status
      $border_class = ($row['is_read_by'] == 0) ? 'border-2 border-blue-500' : 'border border-blue-200';
      ?>
      <a href="adminlist.php" class="block">
    ...
    echo "<p class='text-gray-600 dark:text-gray-300'>No Admin Login Requests found.</p>";
  }
  ?>
</div>
<?php } ?>
    ...
    <?php if ($filter == 'course') { ?>
      <div id="course-notifications" class="space-y-4">
        ...
        ?>
          <a href="course_request_logs.php" class="block">
...
<?php if ($filter == 'calendar') { ?>
  <div id="calendar-notifications" class="space-y-4">
    ...
    ?>
      <a href="academic_calendar.php" class="block">
        <div class="p-6 bg-white rounded-2xl shadow hover:shadow-lg transition-all duration-200 dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-gray-700 <?php echo $border_class; ?>">
          <div class="flex justify-between items-center mb-2">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-100">
              📅 Academic Calendar
...
if ($compResult->num_rows > 0) {
    while ($comp = $compResult->fetch_assoc()) {
        ...
        $collegeCheckQuery = "SELECT * FROM college_competitions WHERE competition_id = $comp_id";
