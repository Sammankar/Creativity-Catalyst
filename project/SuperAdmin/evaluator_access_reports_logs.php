...
<div class="bg-gradient-to-r from-blue-400 to-teal-400 p-6 shadow-lg rounded-lg mb-6">
    ...
    <a href="evaluator_export_logs.php" class="bg-red-500 text-white px-5 py-3 rounded-lg shadow-xl mt-3 inline-block hover:bg-red-600 transition-all duration-300">Export as PDF</a>
...
<div id="accessUpdatePopup" class="fixed top-0 left-0 w-full h-full bg-black bg-opacity-60 flex justify-center items-center hidden">
    <div class="bg-white p-6 rounded-lg shadow-lg w-80">
        ...
        function expandReport(reportId) {
            window.location.href = "evaluator_report_details.php?report_id=" + reportId;
        }
    ...
    document.addEventListener("DOMContentLoaded", function () {
        document.querySelectorAll(".give-access-btn, .restrict-access-btn").forEach(button => {
            button.addEventListener("click", function () {
                ...
                fetch("update_evaluator_report_access_status.php", {
                    method: "POST",
    ...
    function openChat(reportId) {
        // Super-evaluator's user_id is taken from the session
        const superevaluatorId = <?php echo $_SESSION['user_id']; ?>; // Fetch from session
        
        // Fetch evaluator's user_id from the report data
        const evaluatorUserId = <?php echo $report['user_id']; ?>; // evaluator user_id associated with this report
        ...
        document.getElementById("chat-container").setAttribute("data-evaluator-id", evaluatorUserId);  // Store evaluator's ID
    ...
    fetch("evaluator_get_message.php?report_id=" + reportId)
        .then(response => response.json())
        .then(data => {
            ...
            data.forEach(msg => {
                ...
                // If the sender is the logged-in user (Super-evaluator), style the message accordingly
                if (msg.sender_id == <?php echo $_SESSION['user_id']; ?>) {
                    messageElement.classList.add("bg-green-300", "text-black", "ml-auto"); // Super-evaluator's message
                } else {
                    messageElement.classList.add("bg-white-200", "text-black-900", "mr-auto"); // evaluator's message
                }
            ...
            fetch("evaluator_access_reports_logs.php?report_id=" + reportId)
                .then(response => response.json())
                .then(data => {
                    if (data.locked) {
    ...
    function sendMessage() {
        ...
        let superevaluatorId = document.getElementById("chat-container").getAttribute("data-super-evaluator-id"); // Super-evaluator ID from session
        let evaluatorUserId = document.getElementById("chat-container").getAttribute("data-evaluator-id");  // evaluator User ID
        
        if (message === "") return;

        fetch("evaluator_send_message.php", {
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            body: `report_id=${reportId}&super_evaluator_id=${superevaluatorId}&evaluator_user_id=${evaluatorUserId}&message=${message}`
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateAccess(reportId, userId, action) {
        fetch("update_evaluator_report_access_status_v2.php", { // Use the new PHP file here
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            body: `report_id=${reportId}&user_id=${userId}&action=${action}`
        })
...
<script>
    ...
    fetch("evaluator_toggle_lock.php", {
        method: "POST",
