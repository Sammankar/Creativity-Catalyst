...
<div class="bg-gradient-to-r from-blue-400 to-teal-400 p-6 shadow-lg rounded-lg mb-6">
    ...
    <a href="export_logs.php" class="bg-red-500 text-white px-5 py-3 rounded-lg shadow-xl mt-3 inline-block hover:bg-red-600 transition-all duration-300">Export as PDF</a>
...
<div id="accessUpdatePopup" class="fixed top-0 left-0 w-full h-full bg-black bg-opacity-60 flex justify-center items-center hidden">
    <div class="bg-white p-6 rounded-lg shadow-lg w-80">
        ...
        function expandReport(reportId) {
            window.location.href = "report_details.php?report_id=" + reportId;
        }
    ...
    document.addEventListener("DOMContentLoaded", function () {
        document.querySelectorAll(".give-access-btn, .restrict-access-btn").forEach(button => {
            button.addEventListener("click", function () {
                ...
                fetch("update_report_access_status.php", {
                    method: "POST",
    ...
    fetch("get_message.php?report_id=" + reportId)
        .then(response => response.json())
        .then(data => {
            ...
            fetch("Access_reports_logs.php?report_id=" + reportId)
                .then(response => response.json())
                .then(data => {
                    if (data.locked) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateAccess(reportId, userId, action) {
        fetch("update_report_access_status_v2.php", { // Use the new PHP file here
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            body: `report_id=${reportId}&user_id=${userId}&action=${action}`
        })
