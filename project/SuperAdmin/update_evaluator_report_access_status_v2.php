...
include 'sendEvaluatorAccessStatusMail.php'; // Include the mail function
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $currentAccessStatus = ($action === "grant") ? 1 : 0; // Determine new access status

        // Update user access and report access status
        if ($action === "grant") {
            // Grant access
            ...
            // Update the report log with access details
            $updateReport = $conn->prepare("UPDATE evaluator_access_denial_reports SET access_denial_report_status = 1, previous_access_status = ?, current_access_status = ? WHERE report_id = ?");
            $updateReport->bind_param("iii", $previousAccessStatus, $currentAccessStatus, $report_id);
            $updateReport->execute();
        } elseif ($action === "restrict") {
            // Restrict access
            $updateUser = $conn->prepare("UPDATE users SET access_status = 0 WHERE user_id = ?");
            $updateUser->bind_param("i", $user_id);
            $updateUser->execute();
