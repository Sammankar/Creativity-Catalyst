...
function sendAccessChangeEmail($user_id, $changed_by, $new_access, $subject, $body, $warning, $attachments) {
    ...
    $uploadDir = __DIR__ . '/images/evaluator_access_reports/';
    ...
    try {
        ...
        $stmt = $db->prepare("INSERT INTO evaluator_access_denial_reports (user_id, changed_by, old_access, new_access, subject, body, warning, attachment) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        $stmt->execute();
        if ($stmt->error) {
            return "Error inserting into evaluator_access_denial_reports: " . $stmt->error;
        }
        $stmt->close();

        // Capture the report_id generated by the database
        $report_id = $db->insert_id;  // This will give the auto-incremented report_id
        
        $query = "INSERT INTO evaluator_chat_reports (report_id, locked) VALUES (?, 0)";
