...
<div class="grid gap-6 mb-8 md:grid-cols-2 xl:grid-cols-4">
                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b bg-gray-50">
                            ...
                            <th class="px-4 py-3">Competition Details</th>
  ...
  // Fetch full competition details including evaluation_start_date
  $compId = $row['competition_id'];
  $evalQuery = "SELECT evaluation_start_date FROM competitions WHERE competition_id = $compId";
  $evalResult = $conn->query($evalQuery);
  $evalRow = $evalResult->fetch_assoc();
  $evaluationStartDate = new DateTime($evalRow['evaluation_start_date']);

  // Check if current time is before evaluation_start_date
  $isBeforeEvaluation = $currentDateTime < $evaluationStartDate;
?>

<!-- Assign Button -->
<td class="px-4 py-3 text-sm">
  <?php if ($isBeforeEvaluation): ?>
    <a 
      href="assign_evaluator.php?id=<?php echo $row['competition_id']; ?>" 
  ...
  <?php else: ?>
    <button 
      disabled 
      title="TimeOut"
      class="px-4 py-2 bg-gray-300 text-gray-700 font-semibold rounded-full shadow-md cursor-not-allowed">
      Assign
    </button>
  <?php endif; ?>
</td>
...
<?php if ($isBeforeEvaluation): ?>
    <a 
      href="edit_assign_evaluator.php?id=<?php echo $row['competition_id']; ?>" 
      class="px-4 py-2 bg-yellow-400 text-black font-semibold rounded-full shadow-md hover:bg-yellow-500 focus:outline-none focus:ring-2 focus:ring-yellow-300 focus:ring-offset-2">
      Edit
    </a>
  <?php else: ?>
  ...
  <?php endif; ?>
<a 
  href="view_assign_evaluator.php?id=<?php echo $row['competition_id']; ?>" 
  class="px-4 py-2 bg-yellow-400 text-black font-semibold rounded-full shadow-md hover:bg-yellow-500 focus:outline-none focus:ring-2 focus:ring-yellow-300 focus:ring-offset-2">
  View
</a>
</td>
